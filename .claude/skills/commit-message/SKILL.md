...
- Types: `feat`, `fix`, `chore`, `docs`, `refactor`, `test`, `style`, `perf`,
  `build`, `ci`, `revert`.
- Scope: the area the change touches, as the history already does — `i18n`,
  `actions`, `deps`, `dev`. `flask_wiki/` is a flat package, so there is no
  module name to borrow. Check with
  ...
  scope.
- `!` before the colon marks a breaking change: `feat(i18n)!: ...`.
